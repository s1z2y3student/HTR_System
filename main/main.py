...
--dataset= dataset name (bentham, iam, saintgall)
--arch`: network to be used (puigcerver, bluche, flor)
--transform transform dataset files to to the HDF5 file format
...
if __name__ == "__main__":
    ...
    dataset_raw_path = os.path.join("..", "dataset_raw", args.dataset)
    dataset_path = os.path.join("..", "data", f"{args.dataset}.hdf5")
    ...
    if args.transform:
        print(f"{args.dataset} transforming...")
        ds = Dataset(source=dataset_raw_path, name=args.dataset)
        ds.read_partitions()
        ...
        assert os.path.isfile(dataset_path) or os.path.isfile(target_path)
        os.makedirs(output_path, exist_ok=True)

        dtgen = DataGenerator(source=dataset_path,

...
class Dataset():
    """Class that read images and sentences from dataset_raw files"""
    ...
    def read_partitions(self):
        """Read images and sentences from dataset_raw and devide to gt and dt """
        ...
        for pt in self.partitions:
            for batch in range(0, len(self.dataset[pt]['gt']), batch_size):
                ...
                with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
                    r = pool.map(partial(preproc.preprocess, input_size=image_input_size),
    ...
    def _hdsr14_car_a(self):
        """Based on ICFHR 2014 Competition on HDSR"""
    ...
    def check_text(data, max_text_length=128):
        ...
        for i in reversed(range(len(dt['gt']))):
            text = preproc.text_standardize(dt['gt'][i])
